...
@section('content')
    <form action="{{route('backend.post.update', ['post' => $post->slug])}}" method="POST" enctype="multipart/form-data" class="form-horizontal">
        {{csrf_field()}}
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-5">
                        <h4 class="card-title mb-0">
                            ...
                            <small class="text-muted">{{ __('labels.backend.access.post.edit') }}</small>
                        </h4>
                    </div><!--col-->
                </div><!--row-->
                ...
                <div class="row mt-4">
                    <div class="col">
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <input type="text" class="form-control" placeholder="{{__('validation.attributes.backend.access.posts.title')}}" required name="title" value="{{old('title', $post->title)}}">
                            </div><!--col-->
                        </div><!--form-group-->
                    </div><!--col-->
                </div><!--row-->
                            ...
                            <div class="col-md-10">
                                <select name="category[]" id="" class="form-control" multiple>
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{$category->id}}" {{($post->categories()->where('cate_id', $category->id)->exists())? 'selected': ''}}>{{$category->title}}</option>
                                    @endforeach
                                </select>
                            </div><!--col-->
                        </div><!--form-group-->
                ...
                <div class="row mt-4">
                    <div class="col">
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <textarea name="summary" class="form-control post-content">{{$post->summary}}</textarea>
                            </div><!--col-->
                        </div><!--form-group-->
                    </div><!--col-->
                </div><!--row-->
